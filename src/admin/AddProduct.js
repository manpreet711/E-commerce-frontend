...
const AddProduct = () => {
  ...
  const  onSubmit = (event) => {
    ...
    createProduct(user._id,token,formData).then(data => {
      if(data.error){
        ...
        setValues({...values,name:"",description:"",price:"",photo:"",stock:"",loading:false,createdProduct:data.name,getRedirected:true});
      }
  ...
  const errorMessage = () => (
    <div className="alert alert-danger mt-3" style={{display: error ? "" : "none" }}>
      <h4>{error} Failed to create  Product {createdProduct}</h4>
    </div>
  );
        ...
        <form >
          ...
          <div className="form-group">
            ...
            >
              ...
              {categories && 
              categories.map((cate,index) => <option key={index} value={cate._id}>
              {cate.name}</option>)}
            </select>
          </div>
          <div className="form-group">
